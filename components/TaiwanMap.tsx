...
export default function TaiwanMap({ width, height }: TaiwanMapProps) {
  ...
  return (
    <div>
      ...
      <svg id="map-svg" width={width} height={height} viewBox="0 0 450 600">
        <rect x={-2000} y={-2000} width="1000%" height="1000%" fill="#f9f7e8" />
        {/* draw counties */}
        <g ref={g1Ref}>
          {counties.features.map((feature, i) => {
            return (
              <path
                className={`district county-${i + 1}`}
                key={`county-${i}`}
                d={path(feature) || ""}
                // @ts-ignore
                fill={countyColor(feature.properties.countyName)}
